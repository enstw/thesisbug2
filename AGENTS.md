...
- **Not here yet:** `refs` (promote), `refs-snapshot`, `course-init`, and the four protocol files (they carry personal context in the predecessor and must be generalised first). Skills already name some of these (`./fw build`, `./fw refs promote`), so do not report one as run until `./fw help` lists it. The roadmap in `docs/DESIGN.md` gives the order.
