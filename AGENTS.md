...
- **`install.sh` and `scripts/course-init.py`** create a course: fields → local repo → shallow `.framework/` submodule → skeleton from `assets/course-skeleton/` → agent directives (pointer files, skill symlinks) → private GitHub repo. `course-init.py` uses only the standard library because `install.sh` runs it before anything is installed; `--no-github --framework-url <local path>` exercises it offline.
