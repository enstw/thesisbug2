...
1. **APA-zh bibliography** — *decided:* a single `references.bib` per tier, every entry carrying `langid`; the Lua filter groups 中文／西文 at build time. This replaces the predecessor's `references-zh.bib` + `references-en.bib` split, which two tiers would have turned into four files. **Still to prove** against a real APA-zh paper when `build` is rewritten; if the filter cannot group reliably, this comes back here.
