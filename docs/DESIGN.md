...
| **Size** | Predecessor `assets/` is 35 MB and `.agents/` 13 MB, mostly fonts duplicated across `assets/fonts/` and `house-style/assets/fonts/`. | Keep one copy of each font in the framework and reference it from both the Quarto preamble and the deck tokens. Every course repo clones the framework, so its size is paid once per course. |
