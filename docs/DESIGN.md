...
*A course may opt out of tracking the pin.* Since every agent session now starts with `./fw update`, a course that always runs the latest framework gains nothing from the recorded commit except one `chore(framework)` commit per session. Setting `ignore = all` on the submodule in the course's `.gitmodules` is that opt-out: `./fw update` still pulls but leaves the index alone, and `git status` stops reporting the drift. The cost is goal 4 — a handed-in unit is no longer guaranteed to rebuild identically — so it is the course's call, not the default. Such a course should also set `git config submodule.recurse false` locally, or an ordinary `git pull` rewinds `.framework/` to the stale recorded commit until the next `./fw update`.
...
1. **Source acquisition distinguishes transport from evidence (2026-09-20).** `source-kit` routes unattended known-URL retrieval to optional `robust-web-fetch`, and known EZproxy/institutional login or interactive challenges to optional `authenticated-fetch`, whose browser lifecycle is provided by `browser-cdp`. These capabilities stay in the external skill collection. Original PDFs, explicitly requested page prints, Markdown fallbacks, and partial chapter merges retain distinct acquisition records, because a valid PDF container does not establish fulltext identity or claim support. JSON acquisition results supply the actual path, method, final URL, archive timestamp, and missing chapters; session-bearing access parameters stay out of committed records. Unavailable integrations leave explicit pending/blocked work rather than fabricated verification.
